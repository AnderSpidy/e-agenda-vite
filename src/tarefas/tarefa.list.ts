...
class TarefaPaginaListagem implements IPaginaHTML,IPaginaListagem{
  ...
  atualizarTabela(): void {
    ...
    tarefas.forEach(tarefa => {
      ...
      const btnItens = document.createElement("a");
      btnItens.innerText = "Itens";
      btnItens.className = "btn btn-primary me-3"

      btnItens.addEventListener("click", () =>{
        const idSelecionado = tarefa.id;

        //query parameter
        window.location.href = `itens.create.list.html?id=${idSelecionado}`;
      });

      celulaBotoes.appendChild(btnItens);
